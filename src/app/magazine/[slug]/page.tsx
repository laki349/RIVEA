    ...
    <>
      ...
      <main className="flex-1">
        ...
        <header className="border-b border-hairline px-4 py-4">
          <p className="text-[12px] font-bold text-rose">{article.kind}</p>
          {/* AppBar가 h1을 쓰므로 본문 제목은 h2 (앱 전체 규칙) */}
          <h2 className="mt-[6px] text-[21px] font-bold leading-[1.35] text-ink">
            {article.title}
          </h2>
          <p className="mt-[9px] text-[15px] leading-[1.65] text-body">{article.dek}</p>
          <p className="mt-3 text-[12px] text-meta">읽는 데 {article.readMinutes}분</p>
        </header>
        ...
        <article className="px-4 py-5">
          {article.body.map((s, i) => (
            <section key={s.heading} className={i > 0 ? "mt-7" : ""}>
              <h3 className="mb-[10px] text-[17px] font-bold leading-[1.4] text-ink">
                {s.heading}
              </h3>
              {s.paragraphs.map((p, pi) => (
                <p
                  key={pi}
                  className={`text-[16px] leading-[1.75] text-body ${pi > 0 ? "mt-[14px]" : ""}`}
                >
                  {p}
                </p>
              ))}
        ...
        {/* 고지 — 화장품법 표시·광고 관련 */}
        <p className="mx-4 mb-5 border-t border-subtle pt-4 text-[12px] leading-[1.65] text-meta">
        ...
        {relatedRoutines.length > 0 && (
          <section className="border-t border-hairline">
            <h3 className="px-4 pb-3 pt-4 text-[16px] font-bold text-ink">
              이 글에서 다룬 루틴
            </h3>
