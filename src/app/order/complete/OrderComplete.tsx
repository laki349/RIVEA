...
export default function OrderComplete() {
  ...
  return (
    <main className="flex flex-1 flex-col items-center justify-center px-6 pb-24 text-center">
      ...
      <p className="mt-5 text-[21px] font-bold text-ink">주문이 완료됐어요</p>
      ...
      {order ? (
        <>
          ...
          <div className="mt-4 w-full max-w-[320px] rounded bg-subtle px-4 py-3">
            ...
            <div className="mt-[5px] flex justify-between gap-3 text-[16px]">
              <span className="flex-shrink-0 text-meta">주문상품</span>
              <span className="truncate text-ink">{orderSummary(order)}</span>
            </div>
            <div className="mt-[5px] flex justify-between text-[16px]">
      ...
      <div className="mt-8 flex w-full max-w-[320px] flex-col gap-[10px]">
        ...
        <Link
          href="/"
          className="flex h-12 items-center justify-center rounded-cta bg-ink text-[17px] font-medium text-on-ink"
        >
          홈으로
        </Link>
      </div>
